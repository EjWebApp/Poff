...
[] 할일 제목에 - 가 있으면 오류남
[] 처음 실행시 기본값 문구 변경
[] toDoList 추가 기능(생각날때마다 쉽게 집어넣고, 오늘의 계획짤때 바로 쓸수 있게하기) 
   - 입력방법 :  음성녹음&손글씨&키보드 , 지원그리고 
   - 메모 입력 단축키 : 폰의 사이드 버튼으로 
   
# 2026-03-06
[] 시작 후 추가한 일정의 [실제 시간표시]가 나오지 않음
[] 로그인 구현
[] 앱을(웹을) 닫았다 열어도 실행중이었으면 실행중으로 나오기
[] "         "어도 알림은 받기기
